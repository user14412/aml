...
\begin{table}[t]
\centering
\caption{Macro OOD results over six TableShift datasets. DPL-TTA achieves the best macro OOD F1 among MLP-based TTA variants, while FT-Transformer is the strongest overall baseline.}
...
% Suggested Figure 2:
% Macro OOD F1 by method.
% The figure should show ERM-MLP, Simple-TTA, SafeGate-TTA,
% DPL-TTA, Vanilla-PL-TTA, XGBoost, and FT-Transformer.
% The caption should emphasize that DPL-TTA is best only among
% MLP-based TTA variants, while FT-Transformer remains strongest overall.

% Suggested Figure 3:
% Macro OOD balanced accuracy by method.
% The caption should explicitly avoid claiming a significant DPL improvement.
% It should show that FT-Transformer and XGBoost are stronger overall,
% and that Vanilla-PL-TTA has slightly higher macro BAcc than DPL-TTA.
...
\begin{table}[t]
\centering
\caption{Per-dataset OOD balanced accuracy and F1. DPL-TTA has small and dataset-dependent effects.}
...
& ERM BAcc. & ERM F1
& DPL BAcc. & DPL F1
& Vanilla BAcc. & Vanilla F1
& FT-Trans. BAcc. & FT-Trans. F1 \\
\midrule
...
\begin{table}[t]
\centering
\caption{DPL-TTA diagnostics. Adaptation uses only unlabeled OOD features. OOD labels are not used to compute these quantities.}
...
\begin{table}[t]
\centering
\caption{Ablation study on \texttt{brfss\_diabetes}, \texttt{physionet}, and \texttt{acsunemployment}. DPL-TTA is not best on every metric; Vanilla pseudo-labeling has higher macro BAcc, while DPL-TTA has higher macro F1.}
\label{tab:ablation}
\begin{tabular}{lrrr}
\toprule

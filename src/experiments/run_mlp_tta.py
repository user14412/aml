...
from src.methods.tta.simple_binary_tta import (
    SimpleTTAConfig,
    SimpleTTAResult,
    adapt_binary_classifier,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a simple label-free MLP TTA method from pretrained ERM-MLP "
    ...
    parser.add_argument("--tta-pseudo-label-weight", type=float, default=None)
    parser.add_argument("--tta-confidence-threshold", type=float, default=None)
    parser.add_argument(
        "--tta-update-scope",
        type=str,
        default=None,
        choices=["head", "classifier", "last_layer", "all"],
    )
...
def main() -> None:
    ...
    method_name = "MLP-TTA-Simple"
    run_name = args.run_name or datetime.now().strftime("mlp_tta_%Y%m%d_%H%M%S")
    ...
    for dataset_name in datasets:
        ...
        for seed in seeds:
            context.logger.info("Start MLP-TTA dataset=%s seed=%s", dataset_name, seed)
            ...
            rows.append(row)
            context.logger.info(
                "%s source_ood_bacc=%.4f tta_ood_bacc=%.4f delta=%.4f",
                format_row(row),
...
def resolve_tta_config(config: dict[str, Any], args: argparse.Namespace) -> SimpleTTAConfig:
    tta_config = dict(DEFAULT_TTA_CONFIG)
    configured = config.get("tta", {})
    if "simple_mlp" in configured:
        tta_config.update(configured["simple_mlp"])
    ...
    overrides = {
        ...
        "pseudo_label_weight": args.tta_pseudo_label_weight,
    ...
    return SimpleTTAConfig(**tta_config)
...
def run_one_dataset_seed(
    *,
    ...
    tta_result = adapt_binary_classifier(
        adapted_model,
        arrays.X_ood_test,
        config=tta_config,
        device=device,
    ...
    checkpoint_path = checkpoint_dir / f"{dataset_name}_seed{seed}_mlp_tta.pt"
    source_metadata = {
    ...
    row["tta_pseudo_label_weight"] = tta_config.pseudo_label_weight

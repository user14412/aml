...
from src.methods.tta.dpl_tta import DPLTTAConfig, DPLTTAResult, adapt_dpl_binary_classifier
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Distribution-guided Pseudo-Label TTA from pretrained ERM-MLP "
            "checkpoints on TableShift CSV splits."
        )
    )
    ...
    parser.add_argument("--tta-adapt-steps", type=int, default=None)
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["head", "classifier", "last_layer", "bn", "bn_affine", "head_bn", "head_bn_affine", "all"],
    )
    parser.add_argument("--tta-max-pseudo-samples", type=int, default=None)
...
def main() -> None:
    ...
    method_name = "MLP-DPL-TTA"
    run_name = args.run_name or datetime.now().strftime("mlp_dpl_tta_%Y%m%d_%H%M%S")
    ...
    for dataset_name in datasets:
        ...
        for seed in seeds:
            context.logger.info("Start DPL-TTA dataset=%s seed=%s", dataset_name, seed)
            ...
            rows.append(row)
            context.logger.info(
                "%s source_ood_bacc=%.4f dpl_ood_bacc=%.4f delta=%.4f "
                "target_prior=%.4f pseudo_pos=%d pseudo_neg=%d adapted=%.0f",
                format_row(row),
                row["source_ood_balanced_accuracy"],
                row["ood_balanced_accuracy"],
                row["ood_delta_balanced_accuracy"],
                row["tta_target_prior"],
                int(row["tta_num_pseudo_pos"]),
                int(row["tta_num_pseudo_neg"]),
                row["tta_adapted_or_skipped"],
            )
...
def resolve_tta_config(config: dict[str, Any], args: argparse.Namespace) -> DPLTTAConfig:
    tta_config = dict(DEFAULT_DPL_CONFIG)
    configured = config.get("tta", {})
    if "dpl_mlp" in configured:
        tta_config.update(configured["dpl_mlp"])

    overrides = {
        "batch_size": args.tta_batch_size,
        "lr": args.tta_lr,
        "weight_decay": args.tta_weight_decay,
        "adapt_steps": args.tta_adapt_steps,
        "confidence_threshold": args.tta_confidence_threshold,
        "beta": args.tta_beta,
        "prior_shift_threshold": args.tta_prior_shift_threshold,
    ...
    return DPLTTAConfig(**tta_config)
...
def run_one_dataset_seed(
    *,
    ...
    adapted_model, tta_result = adapt_dpl_binary_classifier(
        adapted_model,
        arrays.X_ood_test,
        source_prior=positive_prior(arrays.y_train),
        config=tta_config,
        device=device,
        logger=logger,
    ...
    checkpoint_path = checkpoint_dir / f"{dataset_name}_seed{seed}_mlp_dpl_tta.pt"
    source_metadata = {
